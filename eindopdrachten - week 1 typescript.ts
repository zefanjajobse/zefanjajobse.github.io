...
function checker(numbers:number) {
    if ((numbers % 2) == 0) {
...
// define a string
let defaultStringTS = "Programming is not so cool (Typescript)";
// remove the word "not" and the white space
let newStringTS = defaultStringTS.replace("not ", "");
// log to console
...
// @ts-ignore
let compareStringsTS = 1400 != "Ik woon in Naboo";

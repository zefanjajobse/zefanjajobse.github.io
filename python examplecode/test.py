...
for clean in daily_transactions_split:
  ...
  for items in clean:
    transactions_clean[index].append(items.strip())
  index += 1
...
def color_count(color):
  newlist = 0
  for item_split in thread_sold_split:
    if color == item_split:
      newlist += 1
  return newlist
...
for color in colors:
  print("we sold {count} of {color} today".format(color=color, count=color_count(color)))